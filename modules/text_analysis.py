...
def detect_emotional_tone(text):
    ...
    # Detectar el idioma con langdetect y traducir si es necesario .P
    try:
        if detect(text) != 'en':
            translated_text = GoogleTranslator(source='auto', target='en').translate(text)
            blob = TextBlob(translated_text)
    except Exception as e:
        print(f"Error en la traducción: {e}")  # Agregar registro de errores
        pass  # Si no se puede traducir, continuar con el texto original

    polarity = blob.sentiment.polarity

    # Ajustar umbrales para tonos positivos y negativos
